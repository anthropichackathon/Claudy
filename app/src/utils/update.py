...
def bio_update(claude_response):
    ...
    with open(f"{DATA_ROOT}/bio/BIO.xml", "w") as f:
...
def stm_update(claude_response):
    ...
    for n in range(1, 11):
        stm_template = tags.insert_between_tags(stm_template, f'slot_{n}', st_memory[n - 1])

    with open(f"{DATA_ROOT}/short_term_memory/STM_current.xml", "w") as f:
        f.write(stm_template)
    return

def mtm_temporary(claude_response):
    ...
    # Save the filled templates to the files
    with open(f"{DATA_ROOT}/medium_term_memory/MTM_update.xml", "w") as f:
...
def mtm_update(claude_response):
    ...
    # Save the filled templates to the files
    with open(f"{DATA_ROOT}/medium_term_memory/MTM_current.xml", "w") as f:
        f.write(mtm_template)
    return
